...
## Auth: скользящая 3-дневная сессия через переопределение maxAge кук

Supabase по умолчанию выставляет `maxAge` cookie равным времени жизни JWT (3600 сек = 1 час). Это означает, что кука (и сессия) истекают через 1 час с момента логина, даже если пользователь активен.

**Решение:** два уровня переопределения:
1. В `setAll`-колбэке middleware и server-клиента — `maxAge` принудительно устанавливается в `259200` (3 дня). Это срабатывает при логине и при каждом refresh токена.
2. Rolling-блок в middleware — на каждом запросе от авторизованного пользователя куки `sb-*-auth-token` переставляются с `maxAge = 259200` от текущего момента. Это обеспечивает скользящее окно: пока пользователь заходит хотя бы раз в 3 дня, сессия не истекает.

**Почему не меняли JWT Expiry в Dashboard:** JWT остаётся 1 час — это безопасно. Кука хранит refresh-токен; именно её `maxAge` контролирует время жизни сессии в браузере. Менять JWT Expiry не нужно.
...
## Квесты: NPCDialog через discriminated union, не отдельный компонент

Расширение `NPCDialog` через discriminated union (`TutorialProps | QuestProps`) вместо нового `QuestDialog.tsx`.

**Почему один компонент:** портрет NPC, имя, dot-индикатор прогресса — общий UI между tutorial и quest. Дублирование в отдельном компоненте привело бы к рассинхрону при правке стилей. Discriminated union гарантирует, что quest-mode передаёт только quest-проп, а tutorial-mode — только tutorial-пропс; TypeScript ловит ошибки на этапе компиляции.

## Квесты: acceptQuest использует upsert, не insert

`acceptQuest` делает `upsert` с `onConflict: 'user_id,quest_id'` вместо INSERT.

**Почему:** если пользователь уже отклонил квест (offered → declined workflow в будущем) или повторно открыл диалог, INSERT бы падал с 409. upsert идемпотентен — повторный клик «Принять» не создаёт дубликат.

## Квесты: NPC-настройки в таблице settings, не выделенная таблица

Имя, позиция и портрет NPC хранятся как ключи в `settings` (`npc_name`, `npc_position_x`, `npc_position_y`, `npc_portrait_url`), а не в отдельной таблице `npc_config`.
