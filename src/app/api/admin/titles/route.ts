...
export async function GET() {
  ...
  const { data, error } = await supabase
    .from('titles')
    .select('id, name, min_points, description, privileges, sort_order, created_at')
    .order('sort_order', { ascending: true });

  if (error) {
    return NextResponse.json(
      { error: { code: 'DB_ERROR', message: 'Ошибка загрузки титулов' } },
      { status: 500 },
    );
  }
...
export async function POST(request: Request) {
  ...
  if (error || !title) {
    return NextResponse.json(
      { error: { code: 'DB_ERROR', message: 'Не удалось создать титул' } },
      { status: 500 },
    );
  }
  ...
  return NextResponse.json({ data: { id: (title as { id: string }).id } }, { status: 201 });
}
